...
def generate_sequence_trigram_wfst(word_seq, n=3, self_loop_prob=0.1, unigram_probs=None, final_probs=None, use_sil=False, trigram_probs=None):
    ...
    # Start and end of word states
    start_of_bigram_states = {word: dict() for word in words}# + ["None"]}
    end_of_bigram_states = {word: dict() for word in words}# + ["None"]}

    for word in words:
        lex[word] = lex[word][-1]

    #     current_state = f.add_state()
    #     start_of_bigram_states["None"][word] = current_state
    ...
    #     f.set_final(current_state)
    #     end_of_bigram_states["None"][word] = current_state
